...
function evaluate(expr, env) {
  ...
  switch (expr.type) {
    ...
    case "indexing":
      return logger.set(current_log_index, indexing(expr, env), expr);

    case "var":
      return logger.set(current_log_index, env.get(expr), expr);

    case "new_var":
      return new_var(expr, env);

    case "new_array":
      return logger.set(current_log_index, new_array(expr, env), expr);

    case "assign":
      if (expr.left.type == "var")
        return logger.set(current_log_index, env.set(expr.left, evaluate(expr.right, env), expr), expr);
      if (expr.left.type == "indexing")
        return logger.set(current_log_index, array_assign(expr, env), expr);
      error.assignment_error(expr.left);

    case "unary":
    case "binary":
      return logger.set(current_log_index, apply_op(expr, env), expr);
    ...
    case "call":
      ...
      if (func.dimension == undefined) {
        ...
        return logger.set(current_log_index, eval_function(func, call_args, env), expr);
      }
